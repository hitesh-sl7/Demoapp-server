...
Login.sendLoginData = async(status,data) => {
    try 
    {
        ...
        if(pid == "21" || pid == 21){ 
            ...
            auth_key = new Buffer.from(token).toString('base64');
        }else if(pid == "1234" || pid == 1234){
            token = "9912515294798865:UPskoeQKiQeiGRnB";
            auth_key = new Buffer.from(token).toString('base64');
            ...
            auth_key = new Buffer.from(token).toString('base64');
            if(data.plt == "android"){
                domain = 'https://sandboxmdev.sensfrx.ai/v1/login/android';
            }else{
                domain = 'https://sandboxmdev.sensfrx.ai/v1/login/ios';
            }
